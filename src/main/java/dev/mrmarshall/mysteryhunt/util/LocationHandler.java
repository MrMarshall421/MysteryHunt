...
public class LocationHandler {
	...
	private List<Location> spawningParachute = new ArrayList<>();
	...
	public Location findTreasureLocation(String world, String treasure) {
		...
		while(locationFound == false) {
			...
			//> ProtectionStones check
			if (checkLocation(randomLoc) != null) {
				if (MysteryHunt.getInstance().dependencyManager.getProtectionStones()) {
				...
				//> WorldGuard check
				if (MysteryHunt.getInstance().dependencyManager.getWorldGuard()) {
	...
	public List<Location> getSpawningParachute() {
		return spawningParachute;
	}
}
